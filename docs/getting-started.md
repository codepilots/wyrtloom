...
- [configuration.md](configuration.md) — the `wyrtloom.toml` reference.
- [deployment.md](deployment.md) — operating the dashboard API + web SPA.
- [dashboard-user-guide.md](dashboard-user-guide.md) — using the dashboard as an
  end user.
...
- **`wyrtloom-config`** — the reusable `wyrtloom.toml` schema + loader. Turns the
  ...
  `PluginManifest`, `Capability`). See [configuration.md](configuration.md).
...
- **`wyrtloom-dashboard-api`** — the secure, frontend-agnostic axum HTTP API that
  ...
  behind a default-deny, RBAC-gated router. See [deployment.md](deployment.md).
...
- Operators: see [deployment.md](deployment.md) to build, provision, and run it.
- End users: see [dashboard-user-guide.md](dashboard-user-guide.md).
